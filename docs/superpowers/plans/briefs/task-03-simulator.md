...
- **Handoff (mechanics):** `docs/implementation-handoff.md` § 6 (full `WODSimulator.swift` — reference implementation), § 7 (service contract), § 12 (constraints).
- **Plan (this task):** `docs/superpowers/plans/2026-09-11-wod-counter-implementation.md` → Task 3.
- **Siblings:** Task 1 produced the models — consume `Workout`, `RoundBlock`, `Exercise`, `ExecutionMode` exactly. Task 5 consumes this engine.
...
- All test files must be placed in `WODCounterTests/` (e.g. `WODCounterTests/Sim/`).
...
struct WODSimulator {
    ...
    var totalRepsDone: Int
    var playTarget: Int?

    // methods
    mutating func start()
    mutating func advanceRep() -> TimerEvent
    mutating func startRest() -> TimerEvent
...
- `SessionSnapshot`: Value type snapshot consumed by timer views.
...
- **Multi-block top-time (Fran):** plays each block once (21, 15, 9); finishes after the last block.
...
## Steps
- [ ] **Step 1:** Write `TimerEvent.swift`, `SessionSnapshot.swift`, and `WODSimulator.swift` matching the engine contract and correct semantics above. Use a `struct` (value type), NOT a class, NOT `@Model`.
- [ ] **Step 2:** Write `WODCounterTests/Sim/WODSimulatorTests.swift` covering:
  ...
  - Fran (3 blocks: 21, 15, 9): 3 rounds, auto-stop.
  - DT (1 block ×5): 5 rounds, auto-stop.
  - Helen: 3 rounds.
  - Distanced-only exercise (`reps == nil`) → `effectiveReps == 1`, advances as quota 1.
- [ ] **Step 3:** Run tests under `WODCounterTests/Sim/`, confirm green, commit.
...
- In `WODSimulatorTests`, use `BenchmarkSeed` or hand-crafted `Workout` fixtures. (SwiftData `@Model` objects can be initialized in unit tests).
- BASE commit: HEAD. Diff against it. Done when tests green and the engine compiles against the real models. One commit.

## Acceptance
- All tests pass: AMRAP loops to clock, repeating blocks finish at target, multi-block top-time auto-stops, distanced exercises count as quota 1. Test files in `WODCounterTests/Sim/`. One commit.
